...
from paperModel.FMBEEGCBAM import FMBEEGCBAM
from paperModel.MBEEGCBAM import MBEEGCBAM
from paperModel.MBEEGNet import MBEEGNet
from paperModel.MBShallowConvNet import MBShallowConvNet
...
def train_test(train_conf, results_path):
    ...
    #保存最好的模型 saveModel_Test  TCN消融depth
    filepath = results_path + '/saveModel_Test/{}/checkpoint.h5'.format(model_name)
...
def test(model, train_conf, results_path):
    # 获取训练集、测试集
    X_train, X_val, X_test, y_train, y_val, y_test = getData()
    ...
    model.load_weights(results_path +'/saveModel_Test/{}/checkpoint.h5'.format(model_name))
    ...
    #绘制混淆矩阵
    #draw_confusion_matrix(model, X_test, y_test, model_name)
    draw_confusion(model, X_test, y_test, model_name)
...
def run():
    ...
    train_conf = {'model_name': 'ITransTCNet', 'batch_size':64, 'epochs': 100, 'lr': 0.001}
    ...
    model = getModel(train_conf.get('model_name'))
    test(model, train_conf, results_path)
