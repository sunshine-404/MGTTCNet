...
def Inception(input_layer,Chans, drop_rate,activation):# 12  4
    ...
    # ========================================================================================
    block1 = Conv2D(4, (1, 125), padding='same')(input_layer)
    ...
    # ================================
    block2 = Conv2D(4, (1, 62), padding='same')(input_layer)
    ...
    # ================================
    block3 = Conv2D(4, (1, 31), padding='same')(input_layer)
    ...
    block = AveragePooling2D((1, 4))(block) #后来改成了(1, 8)->(1,4)

    return block

def transformer_encoder(inputs, head_size, num_heads, ff_dim=4, dropout=0.5, feature_dim1=31, feature_dim2=16):
...
def ITCNTrans(n_classes, in_chans, in_samples,
    ...
    # Temporal convolutional network (TCN) 输出（62,32）
    block2 = TCN_block(input_layer=block1, input_dimension=24, depth=2, kernel_size=4, filters=32,dropout=0.3, activation='elu')

    # Transformer_encoder
    for _ in range(num_transformer_blocks):
        block3 = transformer_encoder(block2, head_size, num_heads)
